...
#define GRAVITY 9.81f // Acceleration due to gravity in m/s²

// Example IMU scale factors (adjust based on your configuration)
#define ACCEL_SCALE_FACTOR 16384.0f // For ±2g
#define GYRO_SCALE_FACTOR 131.0f    // For ±250°/s
#define MAG_SCALE_FACTOR 0.14f
...
typedef struct {
    float accel_mps2[3];  // Accelerometer in m/s²
    float gyro_dps[3];    // Gyroscope in degrees/s
    float mag_uT[3];      // Magnetometer in microtesla
} IMUData;
// Full scale ranges
