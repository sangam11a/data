...
uint8_t *Cmd, *id;
HAL_StatusTypeDef def1 = HAL_TIMEOUT,def2=HAL_TIMEOUT,def3=HAL_TIMEOUT;
...
uint8_t Read_Byte_4B(SPI_HandleTypeDef *SPI, uint32_t address) {
	uint8_t cmd[5];
	def1 = HAL_TIMEOUT;def2=HAL_TIMEOUT;def3=HAL_TIMEOUT;
	uint8_t data_r = 0;
	cmd[0] = BYTE_4_READ;
	...
	Cmd = &cmd;
	FM_Enable(SPI);
	def1= HAL_SPI_Transmit(SPI, cmd, 5, 100);
	def2=HAL_SPI_Receive(SPI, &data_r, 1, 200);
	FM_Disable(SPI);
	HAL_Delay(1);
	return data_r;
}
...
//	Write_Enable(SPI);
//	FM_Enable(SPI);
//	def1=HAL_SPI_Transmit(SPI, &cmd, 1, 100);
//	def2=HAL_SPI_Transmit(SPI, command, 4, 200);
//	def3=HAL_SPI_Transmit(SPI, data, 1, 200);
//	FM_Disable(SPI);
//	HAL_Delay(1);
//	return;
//}
void Byte_Write_4B(SPI_HandleTypeDef *SPI, uint32_t address, uint8_t data) {
    ...
    Write_Enable(SPI); // Enable write operations on the flash memory
    FM_Enable(SPI); // Enable the flash memory device

    def1=HAL_SPI_Transmit(SPI, &cmd, 1, 100); // Send the command BYTE_4_PAGE_PROGRAM
    def2=HAL_SPI_Transmit(SPI, command, 4, 200); // Send the address to write to
    def3=HAL_SPI_Transmit(SPI, &data, 1, 200); // Send the data to write

    FM_Disable(SPI); // Disable the flash memory device
    HAL_Delay(1);
}
